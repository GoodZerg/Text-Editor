...
void FuncOne(GLFWwindow* window) {
  ...
  std::string s = "13234242";
  std::string s1 = "3";
  a.push_back(new TextBox(vec1, vec3, s));
  a.push_back(new TextBox(vec1, vec3, s1));
  a.push_back(new Button(vec1, vec3, CloseWindow));
  test.render();

  std::vector<std::string> strings;
  strings.push_back(dynamic_cast<TextBox*>(a[0])->_text);
  strings.push_back(dynamic_cast<TextBox*>(a[1])->_text);
  std::cout << strings[0] << " " << strings[1] << std::endl;
  std::string::size_type sz;
  int pos = std::stoi(strings[1], &sz);

  func::Insert(strings[0], pos);
